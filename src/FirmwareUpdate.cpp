...
String FirmwareVer = FWmain.FirmwareVer;
...
bool FWonce = true; //
void UpdateFW::SetUrlVersion(String input){URL_fw_Version = input;}
void UpdateFW::SetUrlFirmware(String input){URL_fw_Bin = input;}
void UpdateFW::SetVersion(String input){FirmwareVer = input;}
void UpdateFW::SetLedPin(byte Pin ,byte stateOn){httpUpdate.setLedPin(Pin, stateOn);}

 byte UpdateFW::repeatedCall(){
 ...
 byte UpdateFW::FirmwareVersionCheck(void) {
  ...
  if (HttpCode == HTTP_CODE_OK) // if version received
  {
    ...
    if (payload.equals(FirmwareVer)) {
      versionDetec = "Firmware detected ver:" + payload; 
      #ifdef LOOKLINE_UI
    ...
    {
      versionDetec = "New firmware detected ver:" + payload; 
