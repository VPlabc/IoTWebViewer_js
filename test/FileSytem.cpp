...
//################  VERSION  ###########################################
String    VersionFS = "1.1";   // Programme version, see change log at end
//################ VARIABLES ###########################################

AsyncWebServer serverFS(80);
...
void FileSytemFunc::setup() {
  ...
  // ##################### HOMEPAGE HANDLER ###########################
  serverFS.on("/file", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### LOGOUT HANDLER ############################
  serverFS.on("/logout", HTTP_GET, [](AsyncWebServerRequest * request) {
    LogOut();
    request->send(200, "text/html", webpage);
  });

  // ##################### DOWNLOAD HANDLER ##########################
  serverFS.on("/download", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### UPLOAD HANDLERS ###########################
  serverFS.on("/upload", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  serverFS.on("/handleupload", HTTP_POST, [](AsyncWebServerRequest * request) {},
  ...
  serverFS.on("/handleformat", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### STREAM HANDLER ############################
  serverFS.on("/stream", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### RENAME HANDLER ############################
  serverFS.on("/rename", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### DIR HANDLER ###############################
  serverFS.on("/dir", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### DELETE HANDLER ############################
  serverFS.on("/delete", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### FORMAT HANDLER ############################
  serverFS.on("/format", HTTP_GET, [](AsyncWebServerRequest * request) {
  ...
  // ##################### SYSTEM HANDLER ############################
  serverFS.on("/system", HTTP_GET, [](AsyncWebServerRequest * request) {
    Display_System_Info(); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });

  // ##################### IMAGE HANDLER ############################
  serverFS.on("/icon", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->send(FS, "/icon.gif", "image/gif");
  });

  // ##################### NOT FOUND HANDLER #########################
  serverFS.onNotFound(notFound);
  ...
  // ##################### NEW PAGE HANDLER ##########################
  serverFS.on("/newpage", HTTP_GET, [](AsyncWebServerRequest * request) {
    Display_New_Page(); // Build webpage ready for display
    request->send(200, "text/html", webpage);
  });

  serverFS.begin();  // Start the server
  if (!StartupErrors)
...
String HTML_Footer() {
  ...
  page += "<p class='ps'><i>Copyright &copy;&nbsp;D L Bird 2021 Version " +  VersionFS + "</i></p>";
  page += "</footer>";
  page += "</body>";
  page += "</html>";
  return page;
